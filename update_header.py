...
new_header = '''        {/* Header Section */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="bg-gradient-to-br from-[#070b5e] to-[#0a0f82] rounded-2xl shadow-lg border border-[#070b5e]/20 p-8 md:p-10 flex flex-col md:flex-row gap-8 justify-between items-start md:items-center relative overflow-hidden">
          ...
          <div className="relative z-10 bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-5 flex flex-col gap-3 min-w-[320px] shadow-2xl">
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <input 
                ...
                readOnly 
                value={`${window.location.origin}/signup?ref=${profile?.referralCode}`}
                className="flex-1 bg-white/5 border border-white/20 text-sm text-white rounded-lg px-4 py-2.5 outline-none focus:border-white/40 transition-colors"
              />
              <button 
                onClick={copyReferralLink}
                className="bg-[#e03126] hover:bg-[#c9291f] text-white p-2.5 rounded-lg transition-colors shadow-md"
              >
                <Copy size={18} />
              </button>
            </div>
